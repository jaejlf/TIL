...
| 네이티브 메서드 스택 | X | 네이티브 메서드 호출-종료 | X | 
| PC Registers (프로그램 카운터) | X | 스레드 시작-종료 | X | 

> cf. Method 영역은 Java8부터 MetaSpace 영역 즉, Non-Heap 영역에 위치하므로 일반적으로는 GC의 대상이 아니다. 하지만, MetaSpace가 가득차거나 사용되지 않는 클래스 메타 데이터를 정리하는 과정에서 GC가 발생할 수 있다.
...
### Direct Buffer
- Direct Buffer를 이용해 네이티브 메모리 영역을 사용하는 경우
